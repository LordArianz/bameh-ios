...
@objc public enum BMPeekViewActionStyle : Int {
    case `default`
    case selected
    case destructive
}

public struct BMPeekViewAction {
    var title: String
    var style: BMPeekViewActionStyle
    
    public init(title: String, style: BMPeekViewActionStyle){
        self.title = title
        self.style = style
    }
}

@objc open class BMPeekView: UIView {
     ...
     *  whose key will be treated as option title and value is an NSNumber of KCPeekViewActionStyle.
    ...
    @objc open class func viewForController(
        ...
        var options: [BMPeekViewAction]? = nil
        if let menuOptions = menuOptions {
            ...
            for option in menuOptions {
                if let dictionary = option as? NSDictionary,
                    ...
                    let style = BMPeekViewActionStyle(rawValue: styleNumber.intValue) {
                    options?.append(BMPeekViewAction(title: title as String, style: style))
                }
            }
        }
        
        BMPeekView().viewForController(
    ...
    open func viewForController(
        ...
        fromGesture: UILongPressGestureRecognizer,
        shouldHideStatusBar: Bool,
        menuOptions: [BMPeekViewAction]?=nil,
        ...
        case .began:
            let peekView = BMPeekView(frame: window.frame)
            ...
            if let view = window.viewWithTag(peekViewTag) as? BMPeekView {
    ...
    func configureView(viewController: UIViewController, subviewFrame: CGRect, shouldHideStatusBar: Bool, options: [BMPeekViewAction]?=nil, completionHandler: ((Int) -> Void)?=nil, dismissHandler: (()->Void)?=nil) {
        ...
        // Add gesture
        let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(BMPeekView.dismissView))
        tapGestureRecognizer.cancelsTouchesInView = true
        self.addGestureRecognizer(tapGestureRecognizer)
        
        let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(BMPeekView.contentViewPanned(_:)))
        ...
        if let options = options {
            ...
            for index in 0..<options.count {
                ...
                button.addTarget(self, action: #selector(BMPeekView.buttonPressed(_:)), for: .touchUpInside)
                button.tag = index
                button.titleLabel?.font = BMFont.medium(14)
